...
export class WithdrawRequestComponent {
  ...
  constructor(private transService: TransactionService, private cookies: CookieService) { }
  ...
  fetchUsers(): void {
    ...
    this.transService.getAllTransaction().subscribe((data: any) => {
      if(this.cookies.isAdmin()){
        const adminHistory = data.filter(item => item.paymentType === 'withdraw' && item.status === 'pending' )
        this.transInfo = adminHistory;        
        this.filteredTrans = adminHistory;
        this.totalItems = adminHistory.length;
        
      }else{
        const userHistory = data.filter(item => item.userId === this.cookies.decodeToken().userId && item.paymentType === 'withdraw' && item.status === 'pending');
