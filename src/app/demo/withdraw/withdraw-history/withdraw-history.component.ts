  ...
  fetchUsers(): void {
    ...
    this.transactionService.getAllTransaction().subscribe((data: any) => {
      if(this.cookies.isAdmin()){
        const adminHistory = data.filter(item => item.paymentType === 'withdraw' &&  (item.status === 'approved' || item.status === 'rejected'))
        this.transInfo = adminHistory;        
        this.filteredTrans = adminHistory;
        this.totalItems = adminHistory.length;
        
      }else{
        const userHistory = data.filter(item => item.userId === this.cookies.decodeToken().userId && item.paymentType === 'withdraw' &&  (item.status === 'approved' || item.status === 'rejected'));
